...
export async function hartmannPrvniPomocTest() {
  const testName = "hartmannPrvniPomocTest";
  console.log(`--- Starting: ${testName} | ITER ${__ITER} | VU ${__VU} ---`);
  const page = await prepareUI();

  await page.goto(`${__ENV.K6_BASE_URL_HARTMANB2C}/produkty/prvn%C3%AD-pomoc`);
