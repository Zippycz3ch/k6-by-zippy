...
export async function hartmannPristrojeTest() {
  const testName = "hartmannPristrojeTest";
  console.log(`--- Starting: ${testName} | ITER ${__ITER} | VU ${__VU} ---`);
  const page = await prepareUI();

  await page.goto(`${__ENV.K6_BASE_URL_HARTMANB2B}/produkty/zdravotnicke-pristroje`);
